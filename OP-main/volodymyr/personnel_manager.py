...
class PersonnelManager:
    ...
    def view_employees(self, sort_by="last_name"):
        """Перегляд списку співробітників з порахованою зарплатою."""
        sorted_employees = sorted(self.employees, key=lambda emp: getattr(emp, sort_by, ''))
        if sorted_employees:
            for employee in sorted_employees:
                calculated_salary = Payroll.calculate_salary(employee)
                print(f"{employee} → Порахована зарплата: {calculated_salary}")
        else:
    ...
    def calculate_total_salary(self):
        """Підраховує загальну зарплату всіх співробітників з урахуванням коефіцієнтів."""
        total_salary = sum(Payroll.calculate_salary(emp) for emp in self.employees)
        print(f"Загальна зарплата всіх співробітників (з коефіцієнтами): {total_salary}")
